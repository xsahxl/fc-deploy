...
export default class FcDeployComponent {
  ...
  async deploy(inputs: IInputs): Promise<any> {
    ...
    if (needDeployTrigger && needDeployFunction && needDeployService) {
      ...
      if (needDeployService) {
        this.logger.info(
          StdoutFormatter.stdoutFormatter.create('service', resolvedServiceConf.name),
        );
      ...
      if (needDeployFunction) {
        this.logger.info(
          StdoutFormatter.stdoutFormatter.create('function', resolvedFunctionConf.name),
        );
        let resolvedArgs: string;
        if (command === 'function') {
          // deploy function
  ...
  private async handlerInputs(inputs: IInputs): Promise<{ [key: string]: any }> {
    ...
    this.logger.info(
      StdoutFormatter.stdoutFormatter.using(
        'accessKeyID',
        mark(String(this.credentials.AccessKeyID)),
      ),
    );
    this.logger.info(
      StdoutFormatter.stdoutFormatter.using(
        'accessKeySecret',
        mark(String(this.credentials.AccessKeySecret)),
      ),
    );
